...
function ModalDeletar({atualizarTabela, id}) {
    ...
    async function remove() {
      ...
      const response = await abastecimentoV2Req.removeAbastecimetno(id)
      if(response.success){
        atualizarTabela()
        setIsOpen(false)
        toast.success('Abastecimento excluido com sucesso', {
    ...
    return (
        <div>
            ...
            <button onClick={openModal} className="group relative w-24 flex justify-center py-2 px-0 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
            ...
            >
                ...
                <div className="text-center flex flex-col -mt-10">
                    ...
                    <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Tem certeza de que deseja excluir esse Abastecimetno?</h3>
                </div>
