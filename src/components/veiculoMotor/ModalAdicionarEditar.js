...
function ModalAdicionarEditar({atualizarTabela, id, tipo}) {
  ...
  const [vetorCombustiveis, setVetorCombustiveis] = useState([])
    ...
    async function adicionar(e) {
      ...
      const response = await veiculoMotor.addVeiculosMotores(veiculosMotores.descricao, veiculosMotores.ativo, selectOps)
      if(response.success){
        setSelectedOption([])
        setVetorCombustiveis([])
        setVeiculosMotores([])
        atualizarTabela()
        setIsOpen(false)
    ...
    async function atualizar(e) {
      ...
      const response = await veiculoMotor.updateVeiculosMotores(id, veiculosMotores.descricao, veiculosMotores.ativo, selectOps)
      if(response.success){
        // console.log(response.data)
        setSelectedOption([])
        setVetorCombustiveis([])
    ...
    async function pegarCombustiveis() {
      try{
          const response = await veiculoMotor.getCombustiveis()
          setCombustiveis(response.data)
          var comb = response.data.map(function(combustivel, i) {
            return {
              value: combustivel.id,
              label: combustivel.descricao
            }
          })
          setMultiselectOptions(comb)
      }
      catch(err){
          console.log(err);
      }
    ...
    function handleChange(e) {
      setVeiculosMotores({ ...veiculosMotores, [e.target.name]: e.target.value})
      // console.log(veiculosMotores)
    }
    function handleChangeCheckbox(e) {
      setVeiculosMotores({ ...veiculosMotores, [e.target.name]: e.target.checked})
      // console.log(veiculosMotores)
    }
    ...
    function openModal() {
      if(tipo === "editar") {
        ...
        async function pegarVeiculoMotorId() {
          try{
            const response = await veiculoMotor.getVeiculosMotoresID(id)
            // console.log(response.data)
        ...
        async function pegarVeiculoMotorCombustivelId() {
          try{
            const response = await veiculoMotor.getCombustiveisMotorID(id)
            // console.log(response.data)
            var comb2 = response.data.map(function(combustivel, i) {
              return {
                value: combustivel.veiculoCombustivelId,
                label: combustivel.veiculoCombustivel.descricao
              }
            })
            setSelectedOption(comb2)
          }
          catch(err){
              console.log(err);
          }
        }
        pegarVeiculoMotorCombustivelId()
      }
                ...
                <div className="">
                  <form className="mt-2 space-y-3"  onSubmit={tipo === "adicionar" ? adicionar : atualizar}>
                      ...
                      <div>
                          ...
                          <input defaultChecked={veiculosMotores.ativo} onChange={handleChangeCheckbox} type="checkbox" name="ativo" id="ativo" />
                      </div>
