...
function ModalAdicionar({atualizarTabela, id}) {
    ...
    async function adicionar(e) {
      e.preventDefault()
      try{
          const response = await translados.addTranslado(translado.descricao, translado.ativo)
            ...
            >
                ...
                <div className="">
                  <form className="mt-2 space-y-3" onSubmit={adicionar}>
                      <h3 className="block text-gray-700 text-lg font-semibold mb-2">Tipo Translado<span className="font-bold">#Novo Cadastro</span></h3>
                      <div>
                          <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="username">
                              Descrição:
                          </label>
                          <input value={translado.nome} onChange={handleChange} type="text" name="descricao" id="descricao" className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" aria-describedby="nomeHelp" placeholder="Descrição" required />
                      </div>
                      <div>
                          <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="username">
                              Ativo:
                          </label>
                          <input value={translado.ativo ? translado.ativo : false} onChange={handleChangeCheckbox} type="checkbox" name="ativo" id="ativo" />
                      </div>
